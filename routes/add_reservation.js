...
router.post('/reservaciones', async (req, res, next) => {
  ...
  if (departure_date <= arrive_date) {
    errors.push({
      text: 'La fecha de salida no puede ser menor o igual a la fecha de llegada.',
    });
  }
  if (errors.length > 0) {
    res.render('add_reservation', {
    ...
    req.flash(
      'success_msg',
      '¡Reservación añadida con exito! Puede ver sus reservas en la pestaña de su perfil.'
    );
